...
**변경 이력**:
- v1 (2026-04-22): 초안 작성 (Sonnet 작성, 참조 문서 v8/v2/v3/v1 기준).
- v2 (2026-04-26): 참조 문서 일괄 갱신 (v9/v3/v4/v2/v5). 담당자 정정(샤킬라→예병성). 상위 명세 정합성 보강 — `/api/v1` prefix 근거 명시(api_spec_v4 §공통 사항), 더미 응답 출처 명시(db_schema_v3 §초기 데이터), 에러 envelope·예외 핸들러·Literal 타입 정책 신설. ORM 모델 분할 범위 명시(9개 테이블 우선). Alembic·로깅·smoke test·CORS 정책 추가.
...
| 타입 일치 | `db_schema_v3 ↔ api_spec_v4 ↔ Pydantic 스키마` 3방향 필드명 + Literal 값 일치 (불일치 0건) |
...
│   ├── schemas/                      ← Pydantic 응답 DTO (api_spec_v4와 1:1)
│   │   ├── __init__.py
...
| sqlalchemy | 2.0.30 | db_schema_v3 §DB 구성 (SQLAlchemy 2.0 비동기) |
| asyncpg | 0.29.0 | SQLAlchemy 비동기 드라이버 |
| alembic | 1.13.1 | 본 Frame 명세 §3 단독 정의 |
| pydantic | 2.7.1 | api_spec_v4 §공통 사항 (Pydantic v2 serializer) |
| pydantic-settings | 2.2.1 | exception_spec_v2 §6 CFG-CORE-* 설정 로딩 |
| redis | 5.0.4 | db_schema_v3 §DB 구성 (Redis 캐싱) |
| apscheduler | 3.10.4 | feat/batch-scheduler 선반영 |
| statsmodels | 0.14.2 | doc1_v9 §8 (≥0.14 → 정확 버전 고정) |
| scikit-learn | 1.4.2 | doc1_v9 §8 (≥1.4) |
| ruptures | 1.1.9 | doc1_v9 §8 (≥1.1) |
| scipy | 1.11.4 | doc1_v9 §8 (≥1.11) |
| pandas | 2.2.2 | doc1_v9 §8 (≥2.0) |
| numpy | 1.26.4 | doc1_v9 §8 (≥1.24) |
...
>
> **doc1_v9 §8과의 관계**: doc1_v9 §8은 파이프라인 분석 라이브러리만 권장 버전(범위)으로 제시한다. 본 Frame 명세는 이를 정확 버전으로 고정하고, doc1에 부재한 백엔드 패키지(FastAPI, SQLAlchemy, alembic, redis-py 등)를 단독으로 확정한다.
>
...
>
> **ROLLING_WINDOW / CONTAMINATION / RANDOM_STATE 값 출처**: pipeline_output_spec_v5 §파라미터 표 (`settings.py` 위치).
...
| 트랜잭션 단위 | Phase 적재 단위로 단일 트랜잭션. 실패 시 해당 Phase 롤백 (db_schema_v3 §설계 원칙 7, D-17) |
| 월 식별자 변환 | Pydantic **serializer** — `DATE` → `YYYY-MM` (`strftime("%Y-%m")`) 출력. **field_validator** — 입력 시 `YYYY-MM` 형식 강제. **api_spec_v4 §공통 사항 (D-11) 정책 직접 구현** |
| `period` 입력 검증 | `field_validator`에서 `YYYY-MM` 파싱 후 `date(y, m, 1)`로 정규화. `YYYY-MM-DD` 형식이면서 `day != 1`인 경우 `DB-TYPE-001` 발생 (FATAL). **db_schema_v3 §설계 원칙 6 (월초 강제) 직접 대응** |
...
| 항목 | 경로 | 대응 관계 |
|---|---|---|
| 백엔드 Pydantic 스키마 | `app/schemas/` | `api_spec_v4.md` response 1:1 대응 |
...
- **근거**: `team_ai_collab_v6 §3.3` "필드명 드리프트 방지" 원칙.
...
이 Frame 머지 전 PM이 `db_schema_v3.md` → `api_spec_v4.md` → `app/schemas/` 순으로 대조하여 불일치 0건을 확인한다.

| db_schema_v3 컬럼 / 정책 | api_spec_v4 JSON 키 | Pydantic 필드 |
|---|---|---|
...
- **DB CHECK 제약 부재 보완**: db_schema_v3은 CHECK 제약을 두지 않으므로 Pydantic Literal로 입출력 양쪽 모두 강제
- **검증 실패 시**: 응답 422 (`API-VAL-001`)

### 6.4 에러 응답 envelope 스키마

`api_spec_v4 §공통 사항` + `exception_spec_v2 §부록 A` 직접 구현. 모든 에러 응답은 본 envelope를 사용한다.
...
외부 코드 목록은 **api_spec_v4 §에러 코드 정의** (13종) + `INTERNAL_ERROR` 1종을 사용한다. 신규 코드 추가는 `exception_spec_v2 §8` 규칙을 따른다.
...
**근거**: api_spec_v4 §공통 사항 D-11 — 응답 시각은 UTC 고정.
...
api_spec_v4에 없는 frame 단계 신설 엔드포인트. **본 Frame 명세서가 단독 권위 출처**이며, frame 머지 후 api_spec에 추가 반영을 PM에게 요청한다.
...
- api_spec_v4의 모든 경로는 `/api/v1` prefix를 붙여 노출 (예: `/commodities` → `/api/v1/commodities`)
- 라우터 `include_router` 호출은 endpoint 그룹별로 분리 (`commodities`, `anomalies`, `meta`)

### 8.4 전역 예외 핸들러 정책

`exception_spec_v2 §부록 A` 예외 클래스 계층을 `app/core/exceptions.py`에 그대로 정의하고, FastAPI `add_exception_handler`로 전역 등록한다.

```python
# app/core/exceptions.py 골격 (exception_spec_v2 §부록 A 직접 구현)
...
class APIError(ProjectError):             # API-*
    http_status: int
    public_code: str   # api_spec_v4 §에러 코드 정의의 외부 코드
...
|---|---|---|---|
| `APIError` | `exc.http_status` | `exc.public_code` | api_spec_v4 §에러 코드 정의 13종 직접 매핑 |
| `RequestValidationError` (Pydantic) | 400 | `API-VAL-001` | `context.loc` / `context.input` 보존 |
| `PipelineError` / `DBError` / `ExternalAPIError` | 500 | `INTERNAL_ERROR` | **내부 코드 사용자 노출 금지** (exception_spec_v2 §5 API-INT-001) |
...
`app/core/logging.py`에서 `dictConfig`로 초기화. 표준 출력에 **JSON 1줄 포맷**으로 기록 (exception_spec_v2 §부록 A 예시).
...
db_schema_v3는 총 19개 테이블 + 1개 머티리얼라이즈드 뷰를 정의한다. Frame 단계는 그 중 **9개 테이블만 우선 정의**한다. 나머지는 해당 feat 브랜치에서 추가.
...
| `alembic/versions/0002_seed_reference_data.py` | `commodities` 10행 + `segments` 5행 + `external_events` 5행 (db_schema_v3 §초기 데이터) |
...
이상 탐지·ML 투영 등 핵심 ML 로직(doc1_v9 §3~§7)은 본 Frame에서 **구현하지 않는다**. `app/services/`는 빈 상태로 두고 후속 feat 브랜치(`feat/pipeline-phase7-ml`, `feat/api-endpoints` 등)에서 구현.

### 8.12 담당 체계

- **Frame 작성·구현·PR 제출**: 예병성 (sprint_plan §3.2 기준)
- **후속 feat 단계 백엔드 협업자**: 예병성 (계량·DB·API 주도, sprint_plan §4~5), 하대수 (DB 연결·배치 스케줄러·Redis, sprint_plan §5~7), 바게스타니 샤킬라 (분석 수치 패널·온보딩 영역에서 백엔드 응답 협업)
...
| 명세 8종에 부재한 외부 에러 코드 임의 생성 | exception_spec_v2 §8 규칙 위반 |
...
| 전역 예외 핸들러 매핑이 exception_spec_v2 §부록 A와 일치하는가 | ☐ |
...
## 3방향 타입 일치 확인
- [ ] `db_schema_v3.md` ↔ `api_spec_v4.md` ↔ `app/schemas/` 필드명 일치
