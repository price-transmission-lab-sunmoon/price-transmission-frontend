...
**작성일**: 2026-04-28
**관련 문서** (최신 버전 자동 참조 — `abcd_vN.md` 규칙): 에러 코드 인덱스 및 처리 방침 → `exception_spec_vN.md`
**변경 이력**:
- v1 (2026-04-27): 최초 작성. 당시 exception_spec v3 기준 에러 체이닝 설계·상관관계 매트릭스·설계 문제점 분리.
- v2 (2026-04-28): 당시 exception_spec v4 반영. §2.3 출력 예시·§2.5 AI 전달 포맷을 백엔드·프론트 실제 코드 기반으로 교체. §3 상관관계 매트릭스를 PL-* 제거 후 DB→API→FE 전파 경로로 전면 재작성.
- v2 → v3 (2026-05-02): 본문 정정. `reference_audit_report v1` §4 규칙에 따라 외부 참조 표기를 `abcd_vN.md`로 일괄 전환. 본 문서는 이제 `docs/docs_manifest.md`의 버전 해석기에 의해 자동 최신 참조된다.
...
*이 문서는 설계 검토·구현 착수·디버깅 심층 분석 시 참조한다. 반복 코딩 세션에는 `exception_spec_vN.md`만 첨부한다.*
