...
- v2 (2026-05-01): v1 자체 검증 결과 11개 항목 반영
  - **타입 정정**: `selectedPatterns: (1|2|3)[]` → `PrimaryPattern[]` (literals.ts·api_spec_vN 표기 일치)
...
- v3 (2026-05-03): `reference_audit_report_vN.md §4` `abcd_vN.md` 규칙 적용 — 외부 참조 문서 버전 표기 일괄 전환(`_v4`·`_v3` → `_vN`). CLAUDE.md `primary_pattern` / `pattern_types` 타입 불일치 action item 추가. PR 템플릿 명세서 참조 표기 정정.
- v3 (2026-05-04, 자체 검증 반영): SoT 대조 검증 6건 반영 ① §2 `/anomalies/summary` 사용 필드에 `.period`·`.segment_id` 추가 (§3.2 dedup 규칙 사용 필드 정합) ② §1.3 `MethodologyView` → `MethodologyPage`로 통일 (frame_spec_frontend_vN §2 `pages/MainPage.tsx` 컨벤션 준수) ③ Action Items에 CLAUDE.md `anomaly_id: string` ↔ api_spec_vN `integer` 불일치 추가 ④ §3.2 기준 시점 칩 텍스트 단일 문자열 `· ` 구분 형식 명시 (web_plan_vN §3.3) ⑤ §5.1 `FE-API-005` (timeout) 추가 ⑥ Action Items에 로컬 `docs/` 폴더 매니페스트 동기화 필요 항목 추가.
...
| 문서 | 버전 | 참조 목적 | 확인 |
|------|------|-----------|------|
| `api_spec_vN.md §/commodities, §/freshness, §/anomalies/summary` | 최신(`docs_manifest.md` 조회) | 엔드포인트·request·response 필드명 | ☐ |
| `exception_spec_vN.md §2.4 FE-*, §2.3 PARSE-*` | 최신(`docs_manifest.md` 조회) | 이 기능에 해당하는 에러 코드·처리 방침 | ☐ |
| `frame_spec_frontend_vN.md §2` | 최신(`docs_manifest.md` 조회) | 프레임 디렉토리 구조·타입 파일 위치 확인 (CLAUDE.md §3과 불일치 존재 — frame_spec 우선) | ☐ |
| `web_plan_vN.md §3` | 최신(`docs_manifest.md` 조회) | 레이아웃·헤더·배너·필터 바 상세 명세 | ☐ |
...
> 본 명세서 작성 기준일(2026-05-03) 확인된 CLAUDE.md 정합성 문제.  
> 아래 항목은 본 브랜치 PR 병합 전에 별도 단독 커밋(`[CLAUDE.md] Fix primary_pattern type and version refs`)으로 처리한다.

| 항목 | 현황 (CLAUDE.md §6 AnomalyNode) | 정답 | 근거 |
|------|--------------------------------|------|------|
| `primary_pattern` 타입 | `1 \| 2 \| 3` (숫자) | `'pattern1' \| 'pattern2' \| 'pattern3'` (문자열) | `api_spec_vN.md` 실제 응답값 + `literals.ts` `PrimaryPattern` |
| `pattern_types` 타입 | `number[]` | `('pattern1' \| 'pattern2' \| 'pattern3')[]` | 동일 |
| `anomaly_id` 타입 | `string` | `number` (integer) | `api_spec_vN.md §/anomalies/summary` 응답: `"anomaly_id": 142` (정수). `/anomalies/{id}/*` 패널 엔드포인트 동일 |
| CLAUDE.md 헤더 참조 | `api_spec_v4`, `exception_spec_v4` | `api_spec_vN`, `exception_spec_vN` | `abcd_vN.md` 규칙 |
| 로컬 `docs/` 폴더 동기화 | `api_spec_v4`, `db_schema_v3`, `exception_design_v2`, `frame_spec_frontend_v3`, `doc1_technical_pipeline_v9` (구버전); `exception_spec`·`docs_manifest` 부재 | 매니페스트 §1 표 기준 v5/v4/v10 + 매니페스트 입고 | `docs_manifest.md §2.2` 파일 부재 방지 규칙 — 동기화 전까지 본 명세 기반 코드 작업 시 매니페스트 위반 가능 |
...
| 수정 | `src/router/index.tsx` | `/methodology` 라우트 추가. element는 `MethodologyPage` placeholder (실제 구현은 `feat/fe-methodology-tab`) |
...
| API 응답 | `GET /anomalies/summary` | `reference_month`, `total_count`, `prev_month_count`, `count_diff`, `anomalies[].anomaly_id`(integer), `.commodity_id`, `.commodity_name_kr`, `.segment_id`, `.period`, `.confidence_grade`, `.is_new` | 혼합 | Banner 전용. `period`·`segment_id`은 §3.2 dedup 규칙에서 사용 (동일 commodity_id의 멀티 segment·복수 period 케이스 대표 선택) |
...
API 응답 `anomalies[]`은 per-anomaly 배열이지만 배너는 품목 단위로 표시한다 (`web_plan_vN.md §3.2` "이상 탐지 품목명 + 신뢰도 배지").
...
| 배지 클릭 | `selectCommodity(commodity_id)` + `selectAnomaly(대표 anomaly_id)` **동시 호출**. (`web_plan_vN.md §3.2` "해당 품목·최신 이상 시점으로 자동 이동 + 분석 수치 패널 자동 오픈") |
...
- 표시 형식: 단일 문자열, ` · ` 구분 (web_plan_vN §3.3 명시 형식)
  - 예: `data_up_to: "2026-03"`, `next_run_date: "2026-04-15"` → `"2026년 3월 기준 · 다음 갱신 4월 15일 예정"`
- 변환 규칙: `data_up_to` `YYYY-MM` → `"YYYY년 M월 기준"`, `next_run_date` `YYYY-MM-DD` → `"다음 갱신 M월 D일 예정"` (월·일 zero-padding 미적용)
...
이벤트 목록은 `web_plan_vN.md §3.4` 기준 5개 항목 하드코딩. (API `/events` 연동은 `feat/fe-api-connect` 단계에서 전환)
...
> 아래 코드는 `exception_spec_vN.md §8` 기능별 매핑(`feat/fe-layout-filter` 행) 기준.  
> `FE-API-003`, `FE-API-004`, `FE-API-005`, `PARSE-DATE-002`, `PARSE-ENUM-002`는 필수 매핑 외 추가 적용 (해당 엔드포인트에서 실질적으로 발생 가능). 단, `exception_spec_vN.md §6 FE-API-003` 본문은 `feat/fe-layout-filter`를 관련 기능으로 명시하므로 사실상 필수 코드.
...
| `FE-API-005` | 응답 타임아웃 (기본 30초 초과) — 동일 3개 엔드포인트 적용 | FE_TOAST ("응답이 너무 늦습니다.") + 재시도 버튼 |
...
| `exception_spec_vN.md` 미등록 예외 코드 생성 | `(proposed)` 표식으로 PM 제안 후 확정 전까지 임의 코드 금지 |
...
| localStorage / sessionStorage 접근 | `frame_spec_frontend_vN.md §8.10` 절대 금지. Zustand 메모리 상태만 사용 |
| API 응답 snake_case 필드를 camelCase로 변환 | `frame_spec_frontend_vN.md §8.10` camelCase 인터셉터 금지. TypeScript 타입 필드명은 snake_case 유지 |
...
- **기능 번호**: FE-LAY
- **Feature 명세**: `docs/feature_spec_FE-LAY_vN.md` (최신 버전)
- **담당자**: 하대수
...
- [ ] 예외처리 구현 (FE-API-001, FE-API-002, FE-API-003, FE-API-004, FE-API-005, FE-STORE-001, FE-STORE-002, FE-MOCK-001, PARSE-DATE-002, PARSE-ENUM-002)
- [ ] 목업 실행 성공 (기본 mock 활성)
- [ ] 결과 명세 `docs/results/FE-LAY.md` 작성

## 필드명 일치 확인
- [ ] `api_spec_vN.md` JSON 키 ↔ TypeScript 타입 필드명 일치
- [ ] `selectedPatterns` 값이 `'pattern1'|'pattern2'|'pattern3'` 문자열 사용 (숫자 미사용)
- 불일치 항목: {없음 / 목록}

## 예외처리 범위
- 구현한 예외 코드: FE-API-001, FE-API-002, FE-API-003, FE-API-004, FE-API-005, FE-STORE-001, FE-STORE-002, FE-MOCK-001, PARSE-DATE-002, PARSE-ENUM-002
- 신규 제안 코드: 없음
...
## 기타
- **CLAUDE.md §6 타입 불일치 수정 별도 작업**: 본 PR과 별개로 `[CLAUDE.md] Fix primary_pattern type: 1|2|3 → 'pattern1'|'pattern2'|'pattern3'` 단독 커밋 필요. PM 이슈 보고 후 처리. (명세서 §0 Action Items 참조)
- **CLAUDE.md §3 동기화 별도 작업**: 본 PR과 별개로 `[CLAUDE.md] Sync §3 directory structure with frame_spec_vN` 단독 커밋 필요.
- frame_spec_vN ↔ CLAUDE.md §3 불일치는 frame 머지 시점부터 존재. 본 feat에서는 frame_spec 기준으로 작업.
```
