...
- v3 (2026-04-26): 참조 문서 일괄 갱신 (당시 기준 db_schema v3 · api_spec v4 · exception_spec v2 · doc1 v9 · doc3 v11). 백엔드 Frame 명세 v2 양식을 참고하여 명확성 보강 — §1 smoke test 행 추가, §3 "출처" 열로 변경 + 본 Frame 단독 정의 명시, §4 예외 코드 매핑, §6.3 Literal 타입 사용 원칙 신설, §6.4 에러 응답 envelope 스키마 신설, §6.5 날짜·시각 역직렬화 정책 신설, §7.4 smoke test 3건 신설, §8 라우터 prefix 정책·도구 설정·CORS 처리 보강. §8.10 절대 금지 사항을 표 형식으로 재구성. §9 PM 승인 항목 9개로 확장.
- v3 → v4 (2026-05-02): 본문 정정. `reference_audit_report v1` §4 규칙에 따라 외부 참조 표기를 `abcd_vN.md`로 일괄 전환. 잔존 구버전 참조(당시 pipeline_output_spec v5, db_schema v3, exception_spec v2, doc1 v9) 전부 정정. 본 문서는 이제 `docs/docs_manifest.md`의 버전 해석기에 의해 자동 최신 참조되며, 파일명·본문은 `_v4`로 정합.
- v4 → v5 (2026-05-11): `exception_spec_vN` v6 정합 — `FEError` 클래스 §6.4에 명시 (`exception_spec_vN §부록 A` 프론트엔드 구현체 직접 선언, `ApiError extends FEError`로 계층화). §4 시크릿 금지 노트에 백엔드 전용 변수 4종 추가 (`CONTAMINATION_RANGE`, `ZSCORE_WARNING`, `ZSCORE_ALERT`, `PIPELINE_SPEC_VERSION`). §8.2 프록시 결정 브랜치명 `feat/api-endpoints` → `feat/fe-api-connect` 정정. §8.6 D3 구현 브랜치명 6종 전체 정정 (`feat/fe-` prefix 추가, `feat/ml-map` 삭제 후 `feat/fe-panel`에 통합). §8.7 services/ 브랜치명 3종 전체 정정. §10 PR 템플릿 명세 버전 `frame_spec_frontend_v3.md` → `frame_spec_frontend_v5.md` 정정.
- v5 정오 (2026-05-11): 참조 형식 정정 — §5·§8.1·§8.3의 비표준 참조(`frame_spec_backend_*.md`, "백엔드 Frame 명세")를 `frame_spec_backend_vN.md` 형식으로 통일 (`docs_manifest.md` §2.1 `abcd_vN.md` 규칙 준수). 헤더 "작성 기준 문서"에 누락된 `sprint_plan_vN.md`, `team_ai_collab_vN.md`, `frame_spec_backend_vN.md` 3종 추가 (manifest §3.1 forward references 일치). §4 백엔드 전용 변수 목록에 `APP_ENV`, `LOG_LEVEL` 추가 (`frame_spec_backend_vN.md §4` 12종 완전 반영).
...
| 타입 일치 | `db_schema_vN ↔ api_spec_vN ↔ src/types/` 3방향 필드명 + Literal 값 일치 (불일치 0건) |
...
│   │   ├── endpoints.ts             ← api_spec_vN 엔드포인트 18종 경로 상수
│   │   └── error.ts                 ← API 에러 envelope 파서 + ApiError 클래스
│   ├── components/
│   │   ├── layout/
│   │   │   ├── AppShell.tsx         ← 전체 레이아웃 래퍼
│   │   │   ├── Header.tsx           ← 상단 바 자리 표시자 (web_plan_vN §3.3)
│   │   │   ├── FilterBar.tsx        ← 필터 바 자리 표시자 (web_plan_vN §3.4)
│   │   │   └── Panel.tsx            ← 우측 분석 수치 패널 자리 표시자 (web_plan_vN §6)
│   │   └── charts/
│   │       └── .gitkeep             ← 후속 feat 브랜치에서 D3.js 컴포넌트 추가 (§8.6)
│   ├── stores/
│   │   └── useAppStore.ts           ← Zustand 전역 스토어 (타입 선언과 빈 기본값)
│   ├── types/
│   │   ├── commodity.ts             ← 품목·구간 타입 (api_spec_vN /commodities 응답 1:1)
│   │   ├── anomaly.ts               ← 이상 탐지 타입 (api_spec_vN /anomalies/* 응답 1:1, ml_projections 포함)
│   │   ├── timeseries.ts            ← 시계열 공통 envelope 타입 (api_spec_vN §시계열 공통 envelope)
│   │   ├── event.ts                 ← 외부 이벤트 타입 (api_spec_vN /events 응답 1:1)
│   │   ├── meta.ts                  ← /meta/config·/freshness 응답 타입
│   │   ├── error.ts                 ← API 에러 envelope 타입 (§6.4)
│   │   ├── literals.ts              ← Literal 값 상수·유니온 타입 (§6.3)
│   │   └── index.ts                 ← barrel re-export
│   ├── fixtures/
│   │   ├── commodities.json         ← /commodities 목업 응답 (db_schema_vN §commodities 초기 데이터 10행)
│   │   ├── segments.json            ← /segments 목업 응답 (db_schema_vN §segments 초기 데이터 5행)
│   │   ├── events.json              ← /events 목업 응답 (db_schema_vN §external_events 초기 데이터 5행)
...
| react | 18.3.1 | web_plan_vN §11.1 (메이저 18) + 본 Frame 패치 확정 |
| react-dom | 18.3.1 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| typescript | 5.4.5 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| vite | 5.2.11 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| @vitejs/plugin-react | 4.3.0 | 본 Frame 명세 §3 단독 정의 (Vite React 지원) |
| tailwindcss | 3.4.3 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| postcss | 8.4.38 | 본 Frame 명세 §3 단독 정의 (Tailwind 요구) |
| autoprefixer | 10.4.19 | 본 Frame 명세 §3 단독 정의 (Tailwind 요구) |
| d3 | 7.9.0 | web_plan_vN §11.1 (D3.js v7) + 본 Frame 패치 확정 |
| @types/d3 | 7.4.3 | 본 Frame 명세 §3 단독 정의 (D3 타입 정의) |
| zustand | 4.5.2 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| react-router-dom | 6.23.0 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| axios | 1.6.8 | web_plan_vN §11.1 + 본 Frame 패치 확정 |
| @tanstack/react-query | 5.32.0 | 본 Frame 명세 §3 단독 정의 (web_plan_vN §11.1 미명시) |
| date-fns | 3.6.0 | 본 Frame 명세 §3 단독 정의 (web_plan_vN §11.1 미명시) |
...
>
> **web_plan_vN §11.1과의 관계**: web_plan_vN §11.1은 메이저 버전(예: `React 18`, `D3.js v7`)까지만 명시한다. 본 Frame 명세는 이를 정확한 패치 버전으로 고정하고, web_plan에 부재한 패키지(@tanstack/react-query, date-fns, ESLint, Vitest 등)를 단독으로 확정한다.
>
...
|---|---|:---:|---|---|---|
| `VITE_API_BASE_URL` | 백엔드 API Base URL (api_spec_vN §공통 사항 Base URL과 일치) | 필수 | `http://localhost:8000/api/v1` | `console.error` 출력 후 빈 baseURL로 진행, Axios 호출 시 네트워크 에러 발생 | 해당 없음 (프론트 런타임 처리) |
...
> **예외 코드 매핑 정책**: `exception_spec_vN`의 `CFG-CORE-*` 코드는 백엔드(Python) 부팅 시 예외에 한정된다(동 문서 §6 기준). 프론트엔드 환경 변수 누락은 위 표의 "누락 시 동작"에 정의된 방식으로 처리하며 신규 예외 코드를 정의하지 않는다.
>
> **시크릿 금지**: 프론트엔드 환경 변수는 빌드 시 번들에 포함되어 브라우저에서 노출된다. DB 접속 정보·API 키·JWT 시크릿을 기재하지 않는다. 백엔드 전용 변수(`DATABASE_URL`, `REDIS_URL`, `APP_ENV`, `LOG_LEVEL`, `CORS_ALLOWED_ORIGINS`, `ROLLING_WINDOW`, `CONTAMINATION`, `CONTAMINATION_RANGE`, `ZSCORE_WARNING`, `ZSCORE_ALERT`, `RANDOM_STATE`, `PIPELINE_SPEC_VERSION`)는 본 파일에 포함하지 않는다.

---

## 5. DB 연결 방식

> 템플릿 규칙에 따라 프론트엔드 Frame 명세에서는 본 섹션을 삭제한다. DB 연결은 `frame_spec_backend_vN.md` §5를 참조한다.
...
| 항목 | 경로 | 대응 관계 |
|---|---|---|
| 백엔드 Pydantic 스키마 | `app/schemas/` (백엔드 repo) | `api_spec_vN.md` response 1:1 대응 |
| 프론트엔드 TypeScript 타입 | `src/types/` | 백엔드 Pydantic 스키마 1:1 대응 |
| 검증 시점 | `frame/frontend → dev` 머지 직전 PM 게이트 (sprint_plan_vN §10 "S1 후반 완료") | PM이 §6.2 표를 3방향 대조하여 일치 확인 (Literal 값 포함) |

### 6.1 필드명 표기 정책

API 응답 JSON 키를 변형 없이 `snake_case`로 그대로 사용한다.

- **근거**: `team_ai_collab_vN §3.3` "필드명 드리프트 방지" 원칙. DB 컬럼명이 snake_case이므로 API·TypeScript도 동일 표기.
...
`frame/frontend → dev` 머지 직전 PM이 `db_schema_vN.md` → `api_spec_vN.md` → `src/types/*.ts` 순으로 대조하여 불일치 0건을 확인한다.

| db_schema_vN 컬럼 / 정책 | api_spec_vN JSON 키 | TypeScript 타입 |
|---|---|---|
...
- **DB CHECK 제약 부재 보완**: db_schema_vN은 CHECK 제약을 두지 않으므로 백엔드는 Pydantic Literal로, 프론트는 TypeScript 리터럴 유니온으로 입출력 양쪽을 강제한다.

### 6.4 에러 응답 envelope 스키마

`api_spec_vN §공통 사항` + `exception_spec_vN §부록 A` 정책에 따른 응답 형식. 모든 4xx·5xx 응답은 본 envelope를 따른다.
...
```ts
// src/api/error.ts — exception_spec_vN §부록 A 프론트엔드 구현체
...
- **외부 코드 목록**: `api_spec_vN §에러 코드 정의` (13종) + `INTERNAL_ERROR` 1종.
- **인터셉터 동작**: `src/api/error.ts`에서 Axios 응답이 위 형식이면 `ApiError`로 래핑하여 throw. 형식이 아니면 `FEError('NETWORK_ERROR', ...)`로 정규화.
- **신규 코드 추가 금지**: 명세 8종에 부재한 외부 에러 코드를 임의로 생성하지 않는다.

### 6.5 날짜·시각 역직렬화 정책

백엔드는 `api_spec_vN §공통 사항 D-11` 정책으로 날짜를 직렬화한다. 프론트엔드는 그 형식을 그대로 수신하고, 차트·필터에서 필요할 때만 `date-fns`로 파싱한다.
...
- [ ] `commodities.json` 배열 길이 10 (db_schema_vN §commodities 초기 데이터 10행과 일치)
- [ ] `commodities.json` 각 객체에 `commodity_id`, `cluster`, `route_type` 필드 포함, `cluster` 값은 §6.3 Literal 5종 중 하나
- [ ] `segments.json` 배열 길이 5 (db_schema_vN §segments 초기 데이터 5행과 일치)
- [ ] `events.json` 배열 길이 5 (db_schema_vN §external_events 초기 데이터 5행과 일치)
...
| `GET /api/v1/freshness` | 고정값 `{"data_up_to": "2026-03", "next_run_date": "2026-04-15", "last_updated": "2026-04-01T03:00:00Z"}` (`frame_spec_backend_vN.md` §8.1과 일치) |
...
- 프록시 경로(`/api/v1`) 사용 여부는 `feat/fe-api-connect` 브랜치에서 결정한다 (§8.3 참조).

### 8.3 CORS 처리

개발 환경에서 프론트엔드(`:5173`)와 백엔드(`:8000`)가 다른 포트로 동작한다. `frame_spec_backend_vN.md` §8.10에 따라 백엔드 측 `CORS_ALLOWED_ORIGINS` 기본값은 `http://localhost:5173`이다.
...
| `feat/fe-scatter-chart` | 산점도 |
| `feat/fe-raw-timeseries` | 원시 시계열 차트 |
| `feat/fe-panel` | IRF 차트 + 분석 수치 패널 + ML 결과맵 (OI-15 확정 후) |
| `feat/fe-minimap` | 연도별 이상 밀도 미니맵 |
...
```
src/services/
├── timeseries.ts       ← API 시계열 응답 → D3 입력 변환 (feat/fe-stream-chart)
├── anomaly.ts          ← 이상 탐지 패널용 데이터 집계 (feat/fe-panel)
└── filter.ts           ← 필터 상태 → API 쿼리 파라미터 변환 (feat/fe-layout-filter)
```

### 8.8 담당 체계

- **Frame 작성·구현·PR 제출**: 하대수 (sprint_plan_vN §3.2)
- **후속 feat 단계 프론트엔드 협업자**: 하대수 (전체 UI·상태관리 주도, sprint_plan_vN §4~7), 바게스타니 샤킬라 (분석 수치 패널·온보딩 영역, sprint_plan_vN §6~7)

### 8.9 플랫폼 범위

데스크탑 웹 환경을 대상으로 한다 (web_plan_vN §1.3). 모바일 반응형은 1차 출시 범위 외 (web_plan_vN §14).
...
| 명세 8종에 부재한 외부 에러 코드를 `src/api/error.ts`에서 임의 정의 | §6.4 위반, exception_spec_vN §8 규칙 위반 |
...
| 에러 응답 envelope 스키마가 api_spec_vN·exception_spec_vN와 일치하는가 | ☐ |
...
## 개요
- **브랜치**: frame/frontend
- **Frame 명세**: `docs/frame_spec_frontend_v5.md`
- **담당자**: 하대수
...
## 3방향 타입 일치 확인
- [ ] `db_schema_vN.md` ↔ `api_spec_vN.md` ↔ `src/types/` 필드명 일치
