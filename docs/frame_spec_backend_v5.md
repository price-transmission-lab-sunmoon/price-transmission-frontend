...
- v3 → v4 (2026-05-11): `exception_spec_vN` v6 정합 — `PipelineError` 클래스 및 `PL-*` 코드 참조 완전 제거 (v4에서 계량경제학 파이프라인은 예외처리 대상 외로 확정), `ParseError` 클래스 §8.4에 추가 (exception_spec v4 신설 클래스). §4 환경 변수 표에 `ZSCORE_WARNING`, `ZSCORE_ALERT`, `CONTAMINATION_RANGE`, `PIPELINE_SPEC_VERSION` 4종 추가 (`pipeline_output_spec_vN` §파라미터 표 출처). §8.5 로깅 예시 코드를 유효한 `DB-TX-001`로 교체 (구 `PL-P2-003`는 제거된 도메인). §8.6 브랜치명 `feat/redis-cache` → `feat/be-redis` 정정 (feature_dev_list_vN 기준). §8.7 서비스 파일명 `anomaly_detail.py` → `anomaly_panel.py` 정정 (feature_spec_API-PANEL_vN §1.3 확정). §10 PR 템플릿 내 명세 버전 참조 `frame_spec_backend_v2.md` → `frame_spec_backend_v4.md` 정정.
...
| `ZSCORE_ALERT` | Z-score 경보 임계값 | 선택 | `2.5` | `CFG-CORE-003` |
| `PIPELINE_SPEC_VERSION` | 파이프라인 명세 버전 (`docs_manifest.md` §1 `doc1_technical_pipeline` 현재 버전과 일치) | 선택 | `v10` | — |

> **시크릿 금지**: JWT 시크릿·DB 패스워드 등을 코드에 하드코딩 금지. 프론트엔드 전용 변수(`VITE_*`)는 본 파일에 포함하지 않는다.
>
> **ROLLING_WINDOW / CONTAMINATION / CONTAMINATION_RANGE / RANDOM_STATE / ZSCORE_WARNING / ZSCORE_ALERT 값 출처**: `pipeline_output_spec_vN` §파라미터 표 (`settings.py` 위치).
>
> **CONTAMINATION vs CONTAMINATION_RANGE**: `CONTAMINATION`은 단일 실행용 기본값, `CONTAMINATION_RANGE`는 민감도 분석(sensitivity sweep)용 목록. 두 변수는 독립적으로 존재하며, 패널 ML 엔드포인트(`/ml-map`)는 `CONTAMINATION_RANGE`를 사용한다 (`feature_spec_API-PANEL_vN §4`).
...
> **패널 응답 스키마 확장분**: `/stat-series`, `/stat-snapshot`, `/irf`, `/ml-map` 엔드포인트의 응답 스키마는 `app/schemas/anomaly.py` 확장으로 추가되며, `feature_spec_API-PANEL_vN §1.3` 및 `api_spec_vN §패널 엔드포인트`를 기준으로 한다 (`feat/be-api-panel` 브랜치 구현).
...
class ParseError(ProjectError):           # PARSE-*
...
> **PipelineError 제거**: `exception_spec_vN` v4에서 파이프라인(PL-*) 도메인이 전체 제거되었다. 계량경제학 파이프라인 코드는 Python 기본 예외를 그대로 허용하며 별도 에러 클래스를 두지 않는다 (`exception_spec_vN §2 정책 범위` 참조).
...
| `DBError` / `ParseError` / `ExternalAPIError` | 500 | `INTERNAL_ERROR` | **내부 코드 사용자 노출 금지** (exception_spec_vN §5 API-INT-001) |
...
```json
{"ts": "2026-04-26T10:00:00Z", "level": "ERROR", "code": "DB-TX-001",
 "msg": "Phase 적재 중 예외 발생. ORM 세션 롤백.", "table": "stat_timeseries",
 "context": {"phase": "anomaly_results", "commodity_id": "wheat",
             "failed_table": "stat_timeseries", "underlying_error_code": "DB-UNIQ-002"}}
```
...
| (신규) | `mv_anomaly_density_yearly` | `feat/be-redis` 또는 별도 |
...
├── anomaly_panel.py     ← /anomalies/{id}/detail 패널 통합 (4테이블 조인) (feat/be-api-panel)
...
이상 탐지·ML 투영 등 핵심 ML 로직(doc1_vN §3~§7)은 본 Frame에서 **구현하지 않는다**. `app/services/`는 빈 상태로 두고 후속 feat 브랜치(`feat/pipeline-phase7-ml`, `feat/be-api-panel` 등)에서 구현.
...
| `PL-*` 도메인 에러 코드 신규 생성 | exception_spec_vN v4+ 이후 제거된 도메인 |
...
## 개요
- **브랜치**: frame/backend
- **Frame 명세**: `docs/frame_spec_backend_v4.md`
- **담당자**: 예병성
