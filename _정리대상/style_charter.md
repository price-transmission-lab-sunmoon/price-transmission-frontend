...
9. **주석은 기호 없이 문장으로만 쓴다.** `→` `—` `·` `↔` `※` `★` `✓` 등 특수기호 금지 (사람은 귀찮아서 안 씀). 화살표는 "다음", "순으로", "이면"으로, em dash는 쉼표나 마침표로, 가운뎃점 나열은 쉼표나 "와/과"로 풀어쓴다.
   - 예: `// A → B 순으로 탐색` 대신 `// A를 먼저 보고 B 순으로 탐색`
   - 예: `// 회색 band — 이벤트보다 먼저` 대신 `// 회색 band. 이벤트보다 먼저 그린다`
   - 예: `// 상수·타입·스토어` 대신 `// 상수, 타입, 스토어`
   - 단 **UI 렌더링 문자열 속 기호(결측 표시 '—', 구분점 '·', 라벨 '→')는 제품 문구이므로 절대 건드리지 않는다.**
