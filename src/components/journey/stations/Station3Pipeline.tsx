...
// Phase별 호버 설명. '라벨: 내용' 줄은 HoverPanel이 캡션으로 구조화해 렌더한다.
...
export function Station3Pipeline({ active, pipeline, params }: Props) {
  ...
  cols.forEach((col, ci) => {
    ...
    const rowGap = Math.min(1.8, 6 / Math.max(row.length, 1));
    row.forEach((n, j) => {
  ...
  return (
    <group>
      ...
      {nodes.map((n) => {
        ...
        const below = ci % 2 === 0; // 열 단위 교번
        const nodeColor = lerpColor(
          '#059669',
          '#ea580c',
          cols.length > 1 ? ci / (cols.length - 1) : 0,
        );
        return (
          <group key={n.id}>
            <RoundedBox
