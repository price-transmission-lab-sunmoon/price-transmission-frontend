...
const GRADE_COLORS: Record<string, string> = {
  ...
  reference: 'bg-anomaly-reference-bg text-anomaly-reference border-anomaly-reference-border',
};

// 신뢰도 라벨은 confidenceLabel()을 단일 출처로 사용한다.
export function Banner() {
  ...
  return (
    ...
    >
      <span className="text-tertiary text-[12px] shrink-0 font-semibold">이달의 이상</span>
      ...
      <div className="flex items-center gap-2">
        {dedupedAnomalies.map((a) => (
          ...
          >
            {a.is_new && <span className="text-[11px] font-bold text-warning mr-0.5">NEW</span>}
            {a.commodity_name_kr}
            <span className="opacity-70">{confidenceLabel(a.confidence_grade)}</span>
          </button>
        ))}
      </div>

      <span className="text-tertiary text-[12px] shrink-0 ml-1 font-mono">({diffText})</span>
    </div>
  );
}
